    ...
    <tr><td align="center" style="padding:32px 12px;">
      <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="600" class="container"
        ...
        <tr><td style="padding:40px 48px 0 48px;background-color:#FFFFFF;" class="px-mobile">
          ...
          <p style="margin:0 0 20px 0;font-size:16px;line-height:1.65;color:#374151;">Thank you for reaching out — we're glad you did.</p>
          <p style="margin:0 0 32px 0;font-size:16px;line-height:1.65;color:#374151;">Districts like yours are exactly who Education360 was built for. Leaders who recognize that fragmented technology, growing costs, and limited visibility into student outcomes aren't inevitable — and are ready to explore what a more connected, intelligent system could look like.</p>
        ...
        <tr><td style="background-color:#0D1B2A;padding:32px 48px;" class="px-mobile">
          <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr><td align="center">
              ...
              <p style="margin:0;font-size:10px;letter-spacing:2.5px;color:#C9A84C;font-weight:700;text-transform:uppercase;">E360 WORKSHOP &nbsp;&middot;&nbsp; AN INITIATIVE OF XDUCE PUBLIC SERVICES</p>
            </td></tr>
          </table>
        </td></tr>
        ...
        <tr><td style="background-color:#F0EEE9;padding:0 48px 28px 48px;" class="px-mobile">
          ...
          <p style="margin:0;font-size:11px;line-height:1.5;color:#9CA3AF;">
            You received this email because you submitted your interest in the E360 Workshop program.
...
function buildDataEmailHtml(data) {
  const {
    ...
    studentsInfo, notifyEvents, submittedAt,
  } = data;
...
<head>
  <meta charset="UTF-8" />
  <title>New E360 Workshop Submission</title>
  <style type="text/css">
    body,table,td,p { -webkit-text-size-adjust:100%; -ms-text-size-adjust:100%; }
    table,td { mso-table-lspace:0pt; mso-table-rspace:0pt; border-collapse:collapse; }
    body { margin:0; padding:0; background-color:#F0EEE9; }
  </style>
</head>
  ...
  <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="background-color:#F0EEE9;">
    <tr><td align="center" style="padding:32px 12px;">
      <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="560"
        style="width:560px;max-width:560px;background-color:#FFFFFF;border-radius:4px;overflow:hidden;">

        <!-- HEADER -->
        <tr><td style="background-color:#0D1B2A;padding:28px 40px;">
          <p style="margin:0 0 6px 0;font-size:10px;letter-spacing:2px;color:#C9A84C;font-weight:700;text-transform:uppercase;">E360 WORKSHOP · NEW SUBMISSION</p>
        ...
        <tr><td style="padding:32px 40px;">
          <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="border:1px solid #E5E7EB;border-radius:4px;">
            ...
            <tr style="background-color:#F8F6F1;">
              <td colspan="2" style="padding:12px 18px;font-size:10px;letter-spacing:1.5px;color:#6B7280;font-weight:700;text-transform:uppercase;">Contact</td>
            </tr>
            ...
            <tr style="background-color:#F8F6F1;">
              <td colspan="2" style="padding:12px 18px;font-size:10px;letter-spacing:1.5px;color:#6B7280;font-weight:700;text-transform:uppercase;border-top:1px solid #E5E7EB;">School</td>
            </tr>
            ...
            <tr style="background-color:#F8F6F1;">
              <td colspan="2" style="padding:12px 18px;font-size:10px;letter-spacing:1.5px;color:#6B7280;font-weight:700;text-transform:uppercase;border-top:1px solid #E5E7EB;">Interest</td>
            </tr>
            ...
            <tr style="background:#FAFAFA;"><td style="padding:12px 18px;font-size:13px;color:#6B7280;border-top:1px solid #E5E7EB;">Notify Events</td><td style="padding:12px 18px;font-size:14px;color:#0D1B2A;font-weight:600;border-top:1px solid #E5E7EB;">${bool(notifyEvents)}</td></tr>

            ${studentsInfo ? `
            <tr style="background-color:#F8F6F1;">
              <td colspan="2" style="padding:12px 18px;font-size:10px;letter-spacing:1.5px;color:#6B7280;font-weight:700;text-transform:uppercase;border-top:1px solid #E5E7EB;">Student Info</td>
            </tr>
        ...
        <!-- FOOTER -->
        <tr><td style="background-color:#F0EEE9;padding:20px 40px;">
...
export default async function handler(req, res) {
  ...
  try {
    ...
    await transporter.sendMail({
      from:    `"Education360 Workshop" <${process.env.SMTP_USER}>`,
      to:      data.email,
      cc:      BEN_EMAIL,
      subject: 'Thank you for reaching out — Education360 Workshop',
    ...
    await transporter.sendMail({
      from:    `"Education360 Workshop" <${process.env.SMTP_USER}>`,
      to:      BEN_EMAIL,
      subject: `New E360 Workshop submission — ${data.firstName || ''} ${data.lastName || ''} · ${data.schoolName || ''}`.trim(),
      html:    buildDataEmailHtml({ ...data, submittedAt: new Date().toUTCString() }),
    });

    return res.status(200).json({ success: true });
  } catch (err) {
    console.error('send-workshop-email error:', err);
